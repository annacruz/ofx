...
describe OFX::Parser::OFX100 do
  before do
    @ofx = OFX::Parser::Base.new("spec/fixtures/v100.ofx")
    @parser = @ofx.parser
  end

  it "has a version" do
    expect(OFX::Parser::OFX100::VERSION).to eql "1.0.0"
  end
  ...
  it "trims trailing whitespace from headers" do
    headers = OFX::Parser::OFX100.parse_headers("VERSION:100   ")

    expect(headers["VERSION"]).to eql "100"
  end
  ...
  it "knows about all transaction types" do
    ...
    expect(valid_types.sort).to eql OFX::Parser::OFX100::TRANSACTION_TYPES.keys.sort

    valid_types.each do |transaction_type|
      expect(transaction_type.downcase.to_sym).to eql OFX::Parser::OFX100::TRANSACTION_TYPES[transaction_type]
    end
  end
end
